...
![Kathryn's Concept Map](https://www.dropbox.com/s/3s7m0a8fr9m1ul0/Practical_1_concept_mapFINAL.JPG?dl=1)
...
##### The variable **y** contains the mean of **x** which is 5 values randomly generated between 1 and 100.
...
* Use the variable `sec_per_hour` to compute a variable called `sec_per_day` representing the number of seconds per day
  * `sec_per_day` is simply the number of `sec_per_hour` multiplied by 24.
...
* What does the following create? 
  * Specifically, what is the of the data it contains?
    ...
    A vector containing character (a string)

`c(1, 2, 3, "Hi")`

* Would the following be valid?
 
  No, we can't add a numerical value of 1 to a string of characters because they are different data types.
...
Yes we can because logical data types can also become numeric so TRUE is 1 and FALSE is 0.
...
* Hint: We know that we can change the default behavior of a function by changing the default parameters.
  * Which default parameter (param) controls the `x-axis`, `y-axis`, and plot labels?
  x_axis and y_axis vector variable names
  * Which default parameter controls the size of the symbol (dot here)?
  Default size is 1.5 but can be changed using a size function
...
qplot(
  x_axis,
  y_axis,
  main = "My beautiful plot of x_axis versus y_axis",
  xlab = "My x axis",
  ylab = "My y axis",
  color = I("cyan3"),
  size = I(10)
